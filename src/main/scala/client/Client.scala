...
class HttpSocket(path: Path) extends Socket(path: Path) {
  ...
  // remove empty parameters or filters from request
  private def formatRequest(request: Request, method: Method): String = {
    ...
    def convertParams(parameters: A): Option[String] =
      Try(parameters.map((x,y) => s"""$x=$y""".trim).mkString("?", "&", "")) match
        case Success(parameters) => Some(parameters)
        case Failure(e) =>
          println(s"[Client.convertParams] failed to convert parameters from Map to url parameters"); Some("")
    ...
    request match
      ...
      case Request(endpoint, host, parameters, null, null, null) =>
        s"$method $endpoint${convertParams(parameters).get} HTTP/1.1\r\nHost: $host\r\n\r\n"

      case Request(endpoint, host, parameters, filters, null, null) =>
        s"$method $endpoint${convertParams(parameters).get}${convertFilters(filters).get} HTTP/1.1\r\nHost: $host\r\n\r\n"

      case Request(endpoint, host, parameters, null, body, null) =>
        s"$method $endpoint${convertParams(parameters).get} HTTP/1.1\r\nHost: $host\r\nContent-Type: application/json\r\nContent-Length: ${body.length}\r\n\r\n$body"

      case Request(endpoint, host, parameters, null, body, auth) =>
        s"""$method $endpoint${convertParams(parameters).get}
        HTTP/1.1\r\nHost: $host\r\n
...
class Socket(path: Path){
  ...
  private val _channel = Try(UnixSocketChannel.open(_socketAddress)) match {
    case Success(channel) => channel
    case Failure(e: IOException) => throw new IOException(s"[Socker]: Could not initiate connection with exception:\n$e")
  }
